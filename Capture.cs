/****************************************************************************
    HockemaPhotoBooth - Capture.cs is derived from the DxSnap DirectShow example from Microsoft. Search below for 'Jeff Hockema' to find my changes.
    ...
    internal class Capture : ISampleGrabberCB, IDisposable
    {
        ...
        private void SetupGraph(DsDevice dev, int iWidth, int iHeight, short iBPP, Control hControl)
        {
            ...
            try
            {
                ...
                // Find the still pin
                //m_pinStill = DsFindPin.ByCategory(capFilter, PinCategory.Still, 0); // I had to comment this out because it was not allowing my Logitech Webcam to function. -Jeff Hockema
        ...
        private void ConfigVideoWindow(Control hControl)
        {
            ...
            //Modified by Jeff Hockema to display proper aspect ratios.
            if (Globals.settingsPreview == true)
            {
            ...
            else
            {
                ...
                {
                    hr = ivw.SetWindowPosition(0, (int)(-((Globals.cameraVideoHeight - (Globals.cameraVideoWidth * (1 / Globals.currentPrintAspect))) / 2)), rc.Right, rc.Bottom + (int)(2 * ((Globals.cameraVideoHeight - (Globals.cameraVideoWidth * (1 / Globals.currentPrintAspect))) / 2)));
                    DsError.ThrowExceptionForHR(hr);
                }
                else
                {
                    hr = ivw.SetWindowPosition((int)(-((Globals.cameraVideoWidth - (Globals.cameraVideoHeight * Globals.currentPrintAspect)) / 2)), 0, rc.Right + (int)(2 * ((Globals.cameraVideoWidth - (Globals.cameraVideoHeight * Globals.currentPrintAspect)) / 2)), rc.Bottom);
                    DsError.ThrowExceptionForHR(hr);
                }
            }
